 ...
 * Plugin URI: https://github.com/PC-Rights/CP-Brute-Force-Login-Protection/
 * Description: Protects your website against brute force login attacks using .htaccess
 * Text Domain: cp-brute-force-login-protection
 * Author (Original): Fresh-Media
 * Author URI (Original): http://fresh-media.nl/
 * Author: PC Rights
 ...
 * License: GPL2
 * 
 * Copyright 2014 (Original)  Fresh-Media
 * Copyright 2019  PC Rights
 * 
 ...
 */
class CPBruteForceLoginProtection {
    ...
    public function init() {
        ...
        load_plugin_textdomain('cp-brute-force-login-protection', false, dirname(plugin_basename(__FILE__)) . '/languages/');
    }
	...
	* Otherwise adds it to Settings tab.
	*
	* @return void
	*/
    ...
    public function showRequirementsErrors() {
        ...
        if (!$status['found']) {
            $this->__showError(__('CP Brute Force Login Protection error: .htaccess file not found', 'cp-brute-force-login-protection'));
        } elseif (!$status['readable']) {
            $this->__showError(__('CP Brute Force Login Protection error: .htaccess file not readable', 'cp-brute-force-login-protection'));
        } elseif (!$status['writeable']) {
            $this->__showError(__('CP Brute Force Login Protection error: .htaccess file not writeable', 'cp-brute-force-login-protection'));
        }
    }
        ...
        if (isset($_POST['IP'])) {
            ...
            if (isset($_POST['block'])) { //Manually block IP
                ...
                if (in_array($IP, $whitelist)) {
                    $this->__showError(sprintf(__('You can\'t block a whitelisted IP', 'cp-brute-force-login-protection'), $IP));
                } elseif ($this->__htaccess->denyIP($IP)) {
                    $this->__showMessage(sprintf(__('IP %s blocked', 'cp-brute-force-login-protection'), $IP));
                } else {
                    $this->__showError(sprintf(__('An error occurred while blocking IP %s', 'cp-brute-force-login-protection'), $IP));
                }
            } elseif (isset($_POST['unblock'])) { //Unblock IP
                if ($this->__htaccess->undenyIP($IP)) {
                    $this->__showMessage(sprintf(__('IP %s unblocked', 'cp-brute-force-login-protection'), $IP));
                } else {
                    $this->__showError(sprintf(__('An error occurred while unblocking IP %s', 'cp-brute-force-login-protection'), $IP));
                }
            } elseif (isset($_POST['whitelist'])) { //Add IP to whitelist
                if ($this->__whitelistIP($IP)) {
                    $this->__showMessage(sprintf(__('IP %s added to whitelist', 'cp-brute-force-login-protection'), $IP));
                } else {
                    $this->__showError(sprintf(__('An error occurred while adding IP %s to whitelist', 'cp-brute-force-login-protection'), $IP));
                }
            } elseif (isset($_POST['unwhitelist'])) { //Remove IP from whitelist
                if ($this->__unwhitelistIP($IP)) {
                    $this->__showMessage(sprintf(__('IP %s removed from whitelist', 'cp-brute-force-login-protection'), $IP));
                } else {
                    $this->__showError(sprintf(__('An error occurred while removing IP %s from whitelist', 'cp-brute-force-login-protection'), $IP));
                }
            }
            ...
            $this->__setDefaultOptions();
            $this->__setHtaccessPath();
            $this->__showMessage(sprintf(__('The Options have been successfully reset', 'cp-brute-force-login-protection'), $IP));
        }
    ...
    public function loginFailed() {
        ...
        if (!in_array($IP, $whitelist)) {
            ...
            if ($this->__options['inform_user']) {
                ...
                $error .= '<br />';
                $error .= sprintf(_n("%d attempt remaining.", "%d attempts remaining.", $remainingAttempts, 'cp-brute-force-login-protection'), $remainingAttempts);
            }
        }
    }
    ...
    public function validateAllowedAttempts($input) {
        if (is_numeric($input) && ($input >= 1 && $input <= 100)) {
            ...
            add_settings_error('bflp_allowed_attempts', 'bflp_allowed_attempts', __('Allowed login attempts must be a number (between 1 and 100)', 'cp-brute-force-login-protection'));
    ...
    public function validateResetTime($input) {
        if (is_numeric($input) && $input >= 1) {
            ...
            add_settings_error('bflp_reset_time', 'bflp_reset_time', __('Minutes before resetting must be a number (higher than 1)', 'cp-brute-force-login-protection'));
    ...
    public function validateLoginFailedDelay($input) {
        if (is_numeric($input) && ($input >= 1 && $input <= 10)) {
            ...
            add_settings_error('bflp_login_failed_delay', 'bflp_login_failed_delay', __('Failed login delay must be a number (between 1 and 10)', 'cp-brute-force-login-protection'));
    ...
    public function validate403Message($input) {
        ...
        if ($this->__htaccess->edit403Message($message)) {
            ...
            add_settings_error('bflp_403_message', 'bflp_403_message', __('An error occurred while saving the blocked users message', 'cp-brute-force-login-protection'));
    ...
    private function __registerOptions() {
        register_setting('cp-brute-force-login-protection', 'bflp_allowed_attempts', array($this, 'validateAllowedAttempts'));
        register_setting('cp-brute-force-login-protection', 'bflp_reset_time', array($this, 'validateResetTime'));
        register_setting('cp-brute-force-login-protection', 'bflp_login_failed_delay', array($this, 'validateLoginFailedDelay'));
        register_setting('cp-brute-force-login-protection', 'bflp_inform_user');
        register_setting('cp-brute-force-login-protection', 'bflp_send_email');
        register_setting('cp-brute-force-login-protection', 'bflp_403_message', array($this, 'validate403Message'));
        register_setting('cp-brute-force-login-protection', 'bflp_htaccess_dir');
    }
    ...
    private function __sendEmail($IP) {
        ...
        $subject = sprintf(__('IP %s has been blocked', 'cp-brute-force-login-protection'), $IP);
        $message = sprintf(__('CP Brute Force Login Protection has blocked IP %s from access to %s on %s', 'cp-brute-force-login-protection'), $IP, get_site_url(), date('Y-m-d H:i:s'));
...
//Instantiate CPBruteForceLoginProtection class
new CPBruteForceLoginProtection();
